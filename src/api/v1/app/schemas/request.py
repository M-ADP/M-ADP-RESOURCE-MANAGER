...
class DiskSpec(BaseModel):
    ...
    size: str = Field(
        ..., description="디스크 크기 (예: 500Mi, 1024Mi)", examples=["1024Mi"]
    )
    mount_path: str = Field(
        default="/data", description="마운트 경로", examples=["/data"]
    )
    storage_class: str = Field(
        default="linstor-pv-fast", description="StorageClass 이름"
    )
...
class ContainerSpec(BaseModel):
    ...
    name: str = Field(
        ...
        description="컨테이너 이름 (RFC 1123: 소문자, 숫자, 하이픈만 허용)",
        examples=["main"],
    )
    image: str = Field(
        ..., min_length=1, description="컨테이너 이미지", examples=["nginx:latest"]
    )
    ports: Optional[List[Annotated[int, Field(ge=1, le=65535)]]] = Field(
        default=None, description="컨테이너 포트 목록 (1-65535)", examples=[[8080]]
    )
    ...
    env: Optional[Dict[str, str]] = Field(
        default=None, description="환경 변수", examples=[{"ENV": "production"}]
    )
    command: Optional[List[str]] = Field(
        default=None, description="컨테이너 실행 명령어"
    )
...
class AppRevisionResourceRequest(BaseModel):
    ...
    cpu: Optional[str] = Field(
        default=None, description="CPU 리소스 (예: 100m, 500m, 1)"
    )
    memory: Optional[str] = Field(
        default=None, description="메모리 리소스 (예: 128Mi, 512Mi, 1024Mi)"
    )
...
class AppRevisionRequest(BaseModel):
    ...
    container_name: Optional[str] = Field(
        default=None, description="수정할 컨테이너 이름 (미지정시 첫 번째 컨테이너)"
    )
    ...
    disk: Optional[AppRevisionDiskRequest] = Field(
        default=None, description="디스크 크기 수정 (증가만 가능)"
    )
    replicas: Optional[int] = Field(
        default=None, ge=1, le=10, description="레플리카 수"
    )
...
class FixedScaleRequest(BaseModel):
    ...
    replicas: int = Field(..., ge=1, le=10, description="고정 레플리카 수")

    model_config = {"json_schema_extra": {"examples": [{"replicas": 3}]}}


class AutoScaleRequest(BaseModel):
    """Auto Scale 설정 요청 (HPA 생성)"""

    min_replicas: int = Field(default=1, ge=1, le=10, description="최소 레플리카 수")
    max_replicas: int = Field(default=5, ge=1, le=20, description="최대 레플리카 수")
    target_cpu_utilization: int = Field(
        default=80, ge=1, le=100, description="CPU 사용률 기준 (%)"
    )
    target_memory_utilization: Optional[int] = Field(
        default=None, ge=1, le=100, description="메모리 사용률 기준 (%) - 선택사항"
    )
...
class AppCreateRequest(BaseModel):
    ...
    containers: List[ContainerSpec] = Field(
        ..., min_length=1, description="컨테이너 스펙 목록"
    )
    replicas: int = Field(default=1, ge=1, le=10, description="레플리카 수")
    labels: Optional[Dict[str, str]] = Field(
        default=None, description="Deployment 레이블"
    )
    annotations: Optional[Dict[str, str]] = Field(
        default=None, description="Deployment 어노테이션"
    )
...
class EnvironmentUpdateRequest(BaseModel):
    ...
    data: Dict[str, str] = Field(
        ..., description="환경 변수 데이터 (key-value) - 기존 데이터를 완전히 교체"
    )
