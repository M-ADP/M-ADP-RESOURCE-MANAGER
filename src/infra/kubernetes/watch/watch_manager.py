...
class WatchManager:
    """
    EventWatcher + PodWatcher의 생명주기를 관리하고 헬스 상태를 노출한다.

    두 Watcher는 공유 asyncio.Queue에 FailureRecord를 push한다.
    consumer task가 큐를 드레인해 FailureStore에 저장함으로써
    ...
    async def _consume(self) -> None:
        """큐를 드레인해 FailureStore에 저장. Watch 루프와 분리해 이벤트 루프 블로킹을 방지한다."""
        while True:
            try:
    ...
    def is_running(self) -> bool:
        return self._event_watcher.state.is_running and self._pod_watcher.state.is_running
