"""CloudDb 도메인 Repository K8s + Vault 구현체 (StatefulSet 기반)"""
...
class K8sCloudDbRepository(CloudDbRepository):
    """CloudDb 도메인 Repository 구현체 (K8s StatefulSet + Vault)"""

    def __init__(
        self,
        statefulset_manager: StatefulSetManager,
    ...
    async def deploy(self, deployment: Deployment) -> Deployment:
        ...
        if existing:
            v1_sts = await self._statefulset_manager.update_container_images(
                name=deployment.name,
                namespace=deployment.namespace,
                containers=containers,
                image_pull_secrets=deployment.image_pull_secrets or None,
            )
            return self._statefulset_to_domain(v1_sts)
        ...
        # StatefulSet에서 serviceName은 headless service 이름
        headless_service_name = f"{deployment.name}-headless"

        v1_sts = await self._statefulset_manager.create_statefulset(
            name=deployment.name,
            namespace=deployment.namespace,
            service_name=headless_service_name,
            replicas=deployment.replicas,
            selector=deployment.selector_labels or {"cloud_db": deployment.name},
            containers=containers,
    ...
    async def find_deployment(self, name: str, namespace: str) -> Optional[Deployment]:
        v1_sts = await self._statefulset_manager.get_statefulset(name, namespace)
        if v1_sts is None:
            return None
        return self._statefulset_to_domain(v1_sts)

    async def undeploy(self, deployment: Deployment) -> bool:
        return await self._statefulset_manager.delete_statefulset(
            deployment.name, deployment.namespace
        )

    async def scale(self, deployment: Deployment, replicas: int) -> Deployment:
        v1_sts = await self._statefulset_manager.scale_statefulset(
    ...
    async def resize_container(
        ...
        v1_sts = await self._statefulset_manager.update_container_resources(
            name=deployment.name,
            namespace=deployment.namespace,
    ...
    async def get_pods(self, deployment: Deployment) -> List[Pod]:
        v1_pods = await self._pod_manager.list_pods(
            namespace=deployment.namespace,
            label_selector=f"cloud_db={deployment.name}",
        )
    ...
    async def get_events(self, deployment: Deployment) -> List[Event]:
        ...
        for event in result.items:
            ...
            if obj.kind == "StatefulSet" and obj.name == deployment.name:
    ...
    def _statefulset_to_domain(self, v1_sts) -> Deployment:
        containers = []
        volumes = []
        ...
        if v1_sts.status:
            status = DeploymentStatus(
                ...
                available_replicas=v1_sts.status.ready_replicas,  # StatefulSet은 available_replicas 없음
                updated_replicas=v1_sts.status.updated_replicas,
            )

        service_account_name = None
        if v1_sts.spec and v1_sts.spec.template and v1_sts.spec.template.spec:
            service_account_name = v1_sts.spec.template.spec.service_account_name

        selector_labels = {}
        if v1_sts.spec and v1_sts.spec.selector:
            selector_labels = v1_sts.spec.selector.match_labels or {}
