...
`KubernetesClient`는 M-ADP Resource Manager Server가 Kubernetes API와 통신하기 위한 **비동기 클라이언트 래퍼 클래스**입니다. 
...
# 클라이언트 초기화
logger = get_logger()
k8s_client = KubernetesClient(k8s_config, logger)
...
```python
from infra.kubernetes.client import get_kubernetes_client
...
from infra.kubernetes.client import get_kubernetes_client, KubernetesClient
...
async def example_with_context_manager():
    ...
    # 자동으로 초기화 및 정리
    async with KubernetesClient(config, logger) as k8s_client:
        namespaces = await k8s_client.core_v1.list_namespace()
...
async def initialize_with_error_handling():
    config = KubernetesConfig()
    logger = get_logger()
    k8s_client = KubernetesClient(config, logger)
...
async def handle_api_errors(k8s_client: KubernetesClient):
    try:
...
async def with_custom_timeout(k8s_client: KubernetesClient):
    try:
        # 30초 타임아웃 설정
